...
export function PromptsPage({ route, prompts, ...actions }: PromptsPageProps) {
  ...
  const matchesInAnyCategory = filterPrompts(prompts, { query, tag, category: null })
  ...
  return (
    <BrowseLayout
      ...
      context={
        category && (
          <>
            {' in '}
            <strong className="font-medium text-zinc-900 dark:text-zinc-100">{CATEGORY_BY_ID[category].name}</strong>
          </>
        )
      }
      activeTag={tag}
    ...
    >
      {visible.length > 0 ? (
        ...
        <EmptyState title="No prompts match your filters." action={<Button onClick={clearFilters}>Clear filters</Button>} />
      )}
    </BrowseLayout>
  )
}
